...
`images/logo-mark.jpg` (아이콘만 정사각형으로 크롭), `images/logo-wide.jpg`
(아이콘+워드마크 전체, 현재 미사용 — 필요 시 활용)를 원본 로고 파일에서 만들었습니다.
헤더·푸터의 브랜드 마크(`.brand-mark`)와 파비콘에 `logo-mark.jpg`를 사용 중입니다.
로고를 교체하려면 같은 파일명으로 덮어쓰면 됩니다.
